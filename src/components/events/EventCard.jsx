...
export default function EventCard({ event, onOpen }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute inset-0 overflow-hidden bg-[#1a1a1a] rounded-xl border border-white/5"
        style={{
          clipPath: CLIP,
          WebkitClipPath: CLIP,
        ...
        <div className="absolute inset-0 p-5 flex flex-col justify-end">
          ...
          <div className="relative z-20 transform transition-all duration-300 xl:translate-y-4 xl:group-hover:translate-y-0">
            ...
            <div className="flex items-end justify-between gap-2 mb-1">
              <h3 className="text-white text-2xl font-black tracking-tight truncate w-full" title={event.name}>
                {event.name}
              </h3>
              {/* Prize - Always visible */}
              <div className="flex flex-col items-end shrink-0">
                <span className="text-[10px] text-gray-400 uppercase tracking-wider">Win</span>
                <span className="text-[#f0af23] font-bold">
                    {typeof event.prize === "number" ? `₹${event.prize}` : event.prize}
                </span>
              </div>
            </div>
            ...
            <div className="transition-all duration-300 
              ...
              <div className="flex items-center justify-between text-xs text-gray-400 pt-2 border-t border-white/10">
                <div className="flex items-center gap-1">
                   <svg className="w-3 h-3 text-[#f0af23]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
